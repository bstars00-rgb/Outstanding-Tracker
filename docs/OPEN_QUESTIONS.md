...
| E22 | Ellis dev | **P0 (CEO)**: `get_payments` must include `paymentConfirmDate` (PM CNFM) and `traderCompCode`; `get_seller_invoices` and `get_traders` must include the managing entity (`controlCompCode` / `controlCompName`) | Automatic record→verify→reconcile check and Seoul/Singapore split | Chain stage 2 and entity split cannot be automated |
